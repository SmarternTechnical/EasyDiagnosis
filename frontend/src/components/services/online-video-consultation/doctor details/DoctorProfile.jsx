...
const DoctorCard = ({ doctor }) => (
  <div className="bg-white rounded-lg p-4 shadow-md flex gap-4 max-w-md">
    <div className="w-24 h-24 bg-teal-100 rounded-lg">
      <img src={`/${doctor.d_image}`} alt="doctor" className="rounded-lg h-full" />
    </div>
    <div className="flex-1">
      <h2 className="text-blue-900 font-semibold text-lg">{doctor.d_name}</h2>
      <p className="text-emerald-500 text-sm">{`EXPERT: ${doctor.expertise}`}</p>
      <p className="text-gray-600 text-sm mt-1">{`${doctor.work_experience}+ years of experience`}</p>
